...
const NUMBER_OF_CLUES_PER_CATEGORY = 6; // The number of clues you will be displaying per category. You can change this number.
...
function handleClickOfPlay ()
{
  // todo set the game up if the play button is clickable
    // Check if the play button is clickable (you might have a game state variable)
  if (isPlayButtonClickable) { // Assuming a boolean flag to manage button state
    ...
    getCategoryIDs()
      .then(categoryIDs => {
        ...
        const categoryPromises = categoryIDs.map(categoryID => getCategoryWithClues(categoryID));

        // Wait for all category data to be fetched
        Promise.all(categoryPromises)
          .then(categoriesWithClues => {
            // 4. Fill the HTML table with the game data
            fillTable(categoriesWithClues);

            // You might want to update the game state, e.g., show the game board
            // and hide the initial setup
          })
          .catch(error => {
 ...
 * - Request as many categories as possible, such as 100. Randomly pick as many categories as given 
 * in the `NUMBER_OF_CATEGORIES` constant, if the number of clues in the category is enough (<= `NUMBER_OF_CLUES` constant).
 */
async function getCategoryIds ()
{
  ...
  return Promise.resolve({
    ...
    clues: [{
      question: `Question 1 for Category ${categoryID}`,
      answer: `Answer 1 for Category ${categoryID}`,
      value: 100
    }, {
      question: `Question 2 for Category ${categoryID}`,
      answer: `Answer 2 for Category ${categoryID}`,
      value: 200
    }]
  });
}
